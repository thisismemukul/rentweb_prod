...
<div class="container d-flex align-items-center flex-column">
                ...
                <img style="border-radius: 50%; height: 300px; width: 300px;" class="masthead-avatar mb-5" src="assets/seller/user/<?php echo ($_SESSION['username']); ?>/profile/<?php echo ($row['profile']); ?>" alt="user" />
                <!-- username -->
                <h4 class="text-lowercase mb-0"><?php echo ($_SESSION['username']); ?></h4>
                <!-- Masthead Heading-->
                <h1 class="masthead-heading text-uppercase mb-0"><?php echo ($row['name']); ?></h1><br>
            </div>
            ...
            <div class="row">
                    ...
                    <div class="col-md-6 col-lg-4 mb-5">
                        <div class="featured__filter mx-auto" data-toggle="modal" data-target="#portfolioModal1work">
                            <img style="display: block;margin-left: 20%;" class="img-fluid" src="assets/seller/user/default.svg" width="200" height="200" alt="upload" />
                        </div>
                    </div>
             <?php 
                      $tableseller = 'seller_'. $username;
                        $results = mysqli_query($con,"SELECT * FROM $tableseller ORDER BY ID DESC") or die("failed to query database".mysqli_error());
                       // $rows = mysqli_fetch_array($results);
                            if($results->num_rows > 0){
                                while($rows = $results->fetch_assoc()){
                                    $imageURL = 'assets/seller/user/admin/items/'.$rows["product_image"];
