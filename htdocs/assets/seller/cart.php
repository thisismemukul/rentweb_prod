            ...
            <div class="row">
                ...
                <?php 
                      $tableseller = 'seller_'. $username;
                        $results = mysqli_query($con,"SELECT * FROM $username WHERE cart ='Y'") or die("failed to query database".mysqli_error());
                       // $rows = mysqli_fetch_array($results);
                            if($results->num_rows > 0){
                                while($rows = $results->fetch_assoc()){
                                    $imageURL = 'assets/seller/user/'.$_SESSION['username'].'/items/'.$rows["product_image"];
